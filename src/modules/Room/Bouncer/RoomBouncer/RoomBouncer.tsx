...
import { useRoomBouncer } from '../hooks/useRoomBouncer';
...
export const RoomBouncer: React.FC<Props> = ({ onReady = noop, onCancel = noop, ...props }) => {
  const cls = useStyles();
  const bouncer = useRoomBouncer(props.roomInfo.slug);
  ...
  const dialogState = (() => {
    if (bouncer.state?.permissionsGranted === true) {
      ...
      return {
        ...
        content: (
          <Text size="small1">
            Your Camera & Microphone Permissions are Off. Please use your Browser's settings to turn
            them On.
          </Text>
        ),
        buttons: [
