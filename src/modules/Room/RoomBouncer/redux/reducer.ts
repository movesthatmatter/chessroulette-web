...
export const agreePermissionsRequestAction = createAction(
  'AgreePermissionsRequest',
  (resolve) => (p: { roomSlug: RoomRecord['slug'] }) => resolve(p)
);
export const grantPermissionsAction = createAction(
  'GrantPermisssions',
  (resolve) => (p: { roomSlug: RoomRecord['slug'] }) => resolve(p)
);
...
const getReadyFlag = (state: Omit<NonNullable<State>, 'ready'>, roomSlug: string) => {
  return !!(
    state &&
    state.permissionsGranted &&
    state.confirmedRoomJoin &&
    state.confirmedRoomJoin.status &&
    state.confirmedRoomJoin.roomSlug === roomSlug &&
...
export const reducer = createReducer(initialState as State, (handleAction) => [
  handleAction(checkRoomAction, (state, { payload }) => {
    ...
    return {
      ...
      // All
      ready: getReadyFlag(nextState, payload.roomSlug),
    };
  }),

  handleAction(agreePermissionsRequestAction, (state, { payload }) => {
    if (!state) {
      return state;
    }
  ...
  handleAction(grantPermissionsAction, (state, { payload }) => {
    if (!state) {
      return state;
    }
