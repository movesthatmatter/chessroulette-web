...
import { addMoveToHistory } from 'src/modules/Room/RoomActivity/activities/AnalysisActivity/lib';
...
const historyWithBranches = nestedMoves1Gen.reduce(
  (prev, nexMove) => addMoveToHistory(prev, nexMove, [2, 0]),
  linearHistory
);
...
const historyWithTwoDifferentBranches = anotherMoveNestedMoves1Gen.reduce(
  (prev, nexMove) => addMoveToHistory(prev, nexMove, [5, 0]),
  historyWithBranches
);
...
const historyWithParallelBranches = anotherNestedMoves1Gen.reduce(
  (prev, nexMove) => addMoveToHistory(prev, nexMove, [2, 1]),
  historyWithBranches
);
...
const historyWithBranches2Gen = nestedMoves2Gen.reduce(
  (prev, nexMove, index) => addMoveToHistory(prev, nexMove, [2, 0, [0, 0]]),
  historyWithBranches
);
