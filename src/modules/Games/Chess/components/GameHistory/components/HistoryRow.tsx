...
export const HistoryRow: React.FC<Props> = ({
  ...
  const renderNestedContent = () => {
    const [focusedMoveIndex, focusedBranchIndex, focusedNestedIndex] = isBranchedHistoryIndex(
      focusedIndex
    )
      ? focusedIndex
      ...
      <>
        ...
        {blackMove?.branchedHistories?.map((branchedHistory, branchIndex) => {
          ...
          return (
            <HistoryList
              history={branchedHistory}
              onRefocus={(nestedIndex) => onFocus([blackMoveLinearIndex, branchIndex, nestedIndex])}
