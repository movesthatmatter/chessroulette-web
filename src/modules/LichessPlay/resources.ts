...
export function getLichessStreamEvent (opts: RequestInit) {
  return new AsyncResultWrapper<NDJsonReader,ApiError>(async () => {
    try {
      const result = await fetch('https://lichess.org/api/stream/event', {method: 'GET', headers: opts.headers})
...
export function getBoardStreamById (id:string, opts: RequestInit){
  return new AsyncResultWrapper<NDJsonReader, ApiError>(async () => {
    try{
      const result = await fetch(`https://lichess.org/api/board/game/stream/${id}`, {method: 'GET', headers: opts.headers}) 
      return new Ok(ndjsonStream(result.body).getReader())
    } catch (e){
      return new Err({type: 'BadRequest', value: e})
    }
...
export function sendAMove(move: string, id:string, opts: RequestInit) {
  return new AsyncResultWrapper<{ok: true}, {ok: false, error : any}>(async () => {
    try {
      await fetch(`https://lichess.org/api/board/game/${id}/move/${move}`, {method: 'POST', headers: opts.headers})
...
export function acceptChallenge( challengeId: string, opts: RequestInit) {
  return new AsyncResultWrapper<NDJsonReader, ApiError>(async () => {
    try {
      const result = await fetch(`https://lichess.org/api/challenge/${challengeId}/accept`, { method: 'POST', headers: opts.headers})
      return new Ok(ndjsonStream(result.body).getReader())
    } catch (e) {
      return new Err({type :'BadRequest', value: e})
    }
...
export function declineChallenge ( challengeId: string, opts: RequestInit){
  return new AsyncResultWrapper<NDJsonReader, ApiError>(async () => {
    try {
      const result = await fetch(`https://lichess.org/api/challenge/${challengeId}/decline`, { method: 'POST', headers: opts.headers}) 
      return new Ok(ndjsonStream(result.body).getReader())
    } catch (e) {
      return new Err({type :'BadRequest', value: e})
    }
